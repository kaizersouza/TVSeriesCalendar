...
//    suspend fun getSeries() =  apiService.getSeries(tmdb.tmdb_API_KEY, "213", air_date_gte, air_date_lte)

    suspend fun getSeries() : Series {
        var series : Series = Series(0, mutableListOf(), 0, 0)

        tmdb_NetworkArray.forEach {
            val tmp : Series = apiService.getSeries(tmdb.tmdb_API_KEY, it, air_date_gte, air_date_lte)
            tmp.results.forEach {
